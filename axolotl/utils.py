...
def restore_checkpoint(ckpt_dir, state, device, train_lora=None):
    if not os.path.exists(ckpt_dir):
        ...
        # Load the ema and model
        state['model'].module.load_state_dict(loaded_state['model'], strict=False)
        state['ema'].load_state_dict(loaded_state['ema'])
        
        # Handle LoRA model structure differences
        if train_lora:
            logging.warning("Reinitializing EMA because LoRA training is enabled. Also updating the model to use the EMA parameters")
            ...
            state['model'] = setup_lora(state['model'])
